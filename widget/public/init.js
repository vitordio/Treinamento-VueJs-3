function init (apiKey) {
  async function handleLoadWidget () {
    ...
    const WIDGET_URL = `http://localhost:8080?api_key=${apiKey}&page=${page}&fingerprint=${fingerprint.visitorId}`
    const config = { method: 'HEAD' }
    const res = await fetch(`https://backend-treinamento-vue3-vitordio.vercel.app/apikey/exists?apikey=${apiKey}`, config)
